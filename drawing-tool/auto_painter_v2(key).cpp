...
void hotkey_dispatcher(const KeySeq & ks) {
    ...
    if (ks.size() == 1) {
        if (ks[0] == SPECIAL_KEY_1) {
            ...
            write_config();
            cout << "Set left-top: " << pt.x << ", " << pt.y << endl;
            return;
        } else if (ks[0] == SPECIAL_KEY_2) {
            GetCursorPos(&pt);
            cfg.region_rb = Point(pt.x, pt.y);
            write_config();
            cout << "Set right-bottom: " << pt.x << ", " << pt.y << endl;
            return;
        }
    }
...
int main() {
    ...
    cout << "Input 'q' to termiante." << endl;
    //cout << "Press Alt+Ctrl+1/2 to set region left-top / right-bottom" << endl;
    //cout << "Press Alt+Number to draw: ";
    cout << "Input Number -1 to set region left-top" << endl;
    cout << "Input Number -2 to set region right-bottom" << endl;
    cout << "Input Number to draw: ";
    while (true) {
        string str;
        cin >> str;
        if (!str.empty() && str.front() == 'q') {
            break;
        }

        int sel_idx = 0;
        try {
            sel_idx = std::stoi(str);
        } catch (...) {
        ...
        if (sel_idx == -1) {
            ...
            if (rune_tasks.count(sel_idx) == 0) {
                cout << "can't find number: " << sel_idx << endl;
            } else {
                execute_drawing(rune_tasks[sel_idx]);
            }
        }
    }
